...
QRsubsets implements the proposed methodology in the paper $\textit{Bayesian Quantile Regression with Subset Selection: A Posterior Summarization  Perspective}$. Specifically, it provides \textbf{Bayesian subset selection} for the linear quantile regression
...
Among the quantile-specific acceptable families, we can investigate marginal variable importance across the entire reponse distribution. This is done by identifyingcovariates for which the marginal importance is large (> $95\%) for at least one quantile
