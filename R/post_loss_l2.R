...
#' the data.
#'
#' @param post_Q_tau \code{S x n} matrix of posterior samples of the conditional quantile function
#' \eqn{Q_{\tau}(x)} at the covariate values \code{XX}
...
#'
#' @details \code{post_loss} is an \code{S\times L} matrix including the posterior distribution of the difference in RSS between the anchor and each subset from
#' the branch and bound filtering. This is used to determine the acceptable family
#' \code{post_loss_raw} is an \code{S\times L} matrix wit the posterior distribution of the RSS for each susbet in the branch and bound filtering
#' \code{ref} is an \code{S x 1} array of RSS for the anchor action
